...
//	}
	...
	public boolean isStatementOk(String statement) {
		if (statement==null) {
			...
			return isOk(Filter2.NOT_CHECKED);
		} else {
			// Checked transaction
			if (!isOk(Filter2.CHECKED)) {
				return false;
			}
	...
	public static class Factory {
		...
		public Factory() {
			...
			this.setAmountFilter(Filter2.EXPENSES+Filter2.RECEIPTS, 0.0, Double.POSITIVE_INFINITY);
			this.setDescriptionMatcher(null);
			this.setCommentMatcher(null);
			this.setNumberMatcher(null);
			this.setStatementFilter(Filter2.CHECKED+Filter2.NOT_CHECKED, null);
			this.setValidAccounts(null);
		}

		/** Constructor. */
		public Factory(Filter2 filter) {
			this.minAmount = filter.minAmount;
			this.maxAmount = filter.maxAmount;
			this.filter = filter.filter;
			this.statementMatcher = filter.statementMatcher;
			this.validAccounts = filter.validAccounts;
			this.validModes = filter.validModes;
			this.validCategories = filter.validCategories;
			this.dateFrom = filter.dateFrom;
			this.dateTo = filter.dateTo;
			this.valueDateFrom = filter.valueDateFrom;
			this.valueDateTo = filter.valueDateTo;
		...
		public void setName(String name){
			if (!NullUtils.areEquals(name, this.name)) {
				this.name = name;
				this.changed = true;
			}
		}
		...
		public void setAmountFilter(int property, double minAmount, double maxAmount) {
			...
			int mask = Filter2.EXPENSES+Filter2.RECEIPTS;
		...
		public void setStatementFilter (int property, TextMatcher statementFilter) {
			if (((property & Filter2.CHECKED) == 0) && (statementFilter!=null)) {
				throw new IllegalArgumentException();
			}
			int mask = Filter2.CHECKED+Filter2.NOT_CHECKED;
		...
		public Filter2 getFilter() {
			...
			filter.statementMatcher = this.statementMatcher;
			filter.validAccounts = this.validAccounts;
			filter.validModes = this.validModes;
			filter.validCategories = this.validCategories;
			filter.dateFrom = this.dateFrom;
			filter.dateTo = this.dateTo;
			filter.valueDateFrom = this.valueDateFrom;
			filter.valueDateTo = this.valueDateTo;
			filter.descriptionMatcher = this.descriptionMatcher;
			filter.commentMatcher = this.commentMatcher;
			filter.numberMatcher = this.numberMatcher;
			return filter;
		}
